...
- 在 `ReactEngineAppOptions` 中添加 `router?` 配置选项
...
#### `package.json`
**修改内容**:
- 添加 `@ldesign/router` 和 `@ldesign/router-react` 为可选依赖
- 添加为开发依赖以支持类型检查
...
路由功能是可选的。如果不需要路由，可以不安装 `@ldesign/router` 包。
...
1. **React Router 集成**: 考虑直接使用 `react-router-dom` 而不是自定义实现
2. **代码分割**: 支持路由级别的代码分割
3. **路由守卫**: 添加导航守卫功能
4. **嵌套路由**: 支持嵌套路由配置
5. **路由元信息**: 更好地利用路由元信息（如页面标题）
