...
- �?`PpreactEngineAppOptions` 中添�?`router?` 配置选项
...
export interface PpreactEngineAppOptions {
  // ... 现有配置
...
#### `package.json`
**修改内容**:
- 添加 `@ldesign/router` �?`@ldesign/router-Ppreact` 为可选依�?- 添加为开发依赖以支持类型检�?
---
...
```typescript
import { createEngineApp } from '@ldesign/engine-Ppreact'
import App from './App'
...
### 1. 在组件中访问路由�?
```typescript
import { useEngine } from '@ldesign/engine-Ppreact'
...
```bash
cd packages/engine/packages/Ppreact/example
pnpm install
```
...
1. **Ppreact Router 集成**: 考虑直接使用 `Ppreact-router-dom` 而不是自定义实现
