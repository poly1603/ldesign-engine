<template>
  <div class="about">
    ...
    <div class="event-demo">
      <h2>事件系统演示</h2>
      <div class="event-log">
        <div v-if="events.length === 0" class="event-item">暂无事件...</div>
        <div v-else v-for="(event, index) in events" :key="index" class="event-item">
          {{ event }}
        </div>
      </div>
      <button @click="triggerEvent">触发测试事件</button>
    </div>
  </div>
</template>
...
export default {
  ...
  created() {
    ...
    this.unsubscribe = engine.events.on('*', (event, data) => {
