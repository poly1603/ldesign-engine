import { w as watch, s as shallowRef, d as defineComponent, u as unref, a as shallowReactive, i as inject, c as computed, p as provide, r as ref, h, g as getCurrentInstance, n as nextTick, b as reactive, e as watchEffect } from './index-CvMGLjMR.js';
...
function convertRouteRecord(route) {
  const vueRoute = {
    path: route.path,
    name: route.name,
  ...
  if (route.beforeEnter) {
    vueRoute.beforeEnter = route.beforeEnter;
  }
  return vueRoute;
}
function createRouter(options) {
  ...
  if (options.scrollBehavior) {
    vueRouterOptions.scrollBehavior = options.scrollBehavior;
  }
  ...
  const vueRouter = createRouter$1(vueRouterOptions);
  const eventEmitter = options.eventEmitter;
  const router = {
    currentRoute: vueRouter.currentRoute,
    getCurrentRoute: () => {
      return {
        value: vueRouter.currentRoute.value
      };
    },
    ...
    getRoutes: () => {
      return vueRouter.getRoutes();
    },
    resolve: (to) => {
      return vueRouter.resolve(to);
    },
    push: async (to) => {
      const result = await vueRouter.push(to);
      if (eventEmitter) {
    ...
    replace: async (to) => {
      const result = await vueRouter.replace(to);
      if (eventEmitter) {
        eventEmitter.emit("router:navigated", {
          to: vueRouter.currentRoute.value
        });
    ...
    beforeEach: (guard) => {
      return vueRouter.beforeEach(guard);
    },
    beforeResolve: (guard) => {
      return vueRouter.beforeResolve(guard);
    },
    afterEach: (hook) => {
      return vueRouter.afterEach(hook);
    },
    onError: (handler) => {
      ...
      try {
        ...
        const router = createRouter({
          ...
          eventEmitter: engine.events ? {
            emit: (event, data) => engine.events?.emit?.(event, data)
          } : void 0
        });
        try {
          if (typeof window !== "undefined") {
            const initialPath = mode === "hash" ? window.location.hash.slice(1) || "/" : window.location.pathname + window.location.search + window.location.hash;
            try {
              await router.vueRouter.replace(initialPath);
            } catch {
            }
            if (mode === "hash") {
              window.addEventListener("hashchange", () => {
                const to = router.vueRouter?.currentRoute?.value;
        ...
        if (app) {
          ...
          engine.events?.once?.("app:created", () => {
            const app2 = engine.getApp?.();
            if (app2) {
              app2.use(router.vueRouter);
              app2.provide("router", router);
            }
          });
        }
        if (engine.setRouter) {
        ...
        engine.events?.emit?.("router:installed", {
          router,
          mode,
          base
        });
...
export { createRouter, createRouterEnginePlugin };
//# sourceMappingURL=index-J9EqgjYy.js.map
