...
- �?`QwikEngineAppOptions` 中添�?`router?` 配置选项
...
#### `package.json`
**修改内容**:
- 添加 `@ldesign/router` �?`@ldesign/router-Qwik` 为可选依�?- 添加为开发依赖以支持类型检�?
---
...
### 1. 在组件中访问路由�?
```typescript
import { useEngine } from '@ldesign/engine-Qwik'
...
1. **Qwik Router 集成**: 考虑直接使用 `Qwik-router-dom` 而不是自定义实现
