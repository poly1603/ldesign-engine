...
export default defineConfig({
  ...
  server: {
    host: '0.0.0.0',
    port: 5181, // 生产环境端口（如果需要在生产环境运行 dev）
  ...
  build: {
    ...
    rollupOptions: {
      output: {
        // 代码分割优化
        manualChunks: {
          'qwik-vendor': ['@builder.io/qwik'],
          'engine-vendor': ['@ldesign/engine-qwik', '@ldesign/engine-core']
        },
        // 文件名包含 hash，便于缓存
  ...
  resolve: {
    alias: [
      {
        find: '@ldesign/engine-qwik',
        replacement: resolve(__dirname, '../../../qwik/src/index.ts')
      },
      {
      ...
      {
        find: '@ldesign/router-qwik',
        replacement: resolve(__dirname, '../../../../../router/packages/qwik/src/index.ts')
      },
      {
        find: '@ldesign/router',
