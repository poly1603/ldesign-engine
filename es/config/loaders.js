...
class EnvironmentConfigLoader {
  constructor(prefix = "VUE_APP_") {
    this.prefix = prefix;
    this.logger = getLogger("EnvironmentConfigLoader");
  }
  load() {
    const config = {};
    try {
...
class JsonConfigLoader {
  constructor(configPath) {
    ...
    this.logger = getLogger("JsonConfigLoader");
  }
  async load() {
    try {
...
class LocalStorageConfigLoader {
  constructor(key) {
    this.key = key;
    this.logger = getLogger("LocalStorageConfigLoader");
  }
  load() {
    try {
...
class CompositeConfigLoader {
  constructor(loaders) {
    ...
    this.logger = getLogger("CompositeConfigLoader");
  }
  async load() {
    let config = {};
...
export { CompositeConfigLoader, EnvironmentConfigLoader, JsonConfigLoader, LocalStorageConfigLoader, MemoryConfigLoader };
