 ...
 * ***********************************
 */
import { reactive, computed, shallowRef, ref, watch } from 'vue';
...
class ReactiveStateManager {
   ...
   */
  getRef(key) {
    return computed(() => this.getNestedValue(this.state, key));
  }
  /**
  ...
  defineComputed(key, definition) {
    ...
    const computedState = def.set ? computed({
      get: def.get,
      set: def.set
    }) : computed(def.get);
  ...
  watch(key, callback, options) {
    ...
    const stopWatcher = watch(sources.length === 1 ? sources[0] : sources, callback, options);
    keys.forEach((k) => {
...
export { ReactiveStateManager, createReactiveStateManager, createSSRStateManager };
