...
class TooltipDirective extends DirectiveBase {
  ...
  cleanupTooltip(el) {
    const handlers = directiveUtils.getData(el, "tooltip-handlers");
    const trigger = directiveUtils.getData(el, "tooltip-trigger");
    const tooltip = directiveUtils.getData(el, "tooltip-element");
    const showTimer = directiveUtils.getData(el, "tooltip-show-timer");
    const hideTimer = directiveUtils.getData(el, "tooltip-hide-timer");
    if (showTimer) {
      clearTimeout(showTimer);
    }
    if (hideTimer) {
...
const vTooltip = defineDirective(new TooltipDirective());

export { TooltipDirective, vTooltip as default, vTooltip };
