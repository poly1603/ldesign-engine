...
import { ReactiveStateManager } from '../state/reactive-state.js';
...
class PluginSharedStateManager {
  ...
  accessSharedState(requestingPlugin, ownerPlugin, key) {
    ...
    if (entry.metadata.readonly) {
      return computed(() => entry.value.value);
    }
    return entry.value;
  }
  /**
...
function usePluginState(pluginName, key, defaultValue) {
  ...
  const watchState = (callback) => {
    const stopWatch = watch(state, callback);
    return stopWatch;
  };
  return {
    state,
...
export { PluginSharedStateManager, createPluginSharedStateManager, usePluginState };
//# sourceMappingURL=plugin-shared-state.js.map
