      ...
      <div class="error-overview">
        <div class="overview-stats">
          ...
          <div class="stat-card warning">
            ...
            <div class="stat-content">
              <div class="stat-value">{{ errorStats.warnings }}</div>
          ...
          <div class="stat-card success">
            ...
            <div class="stat-content">
              <div class="stat-value">{{ errorStats.recovered }}</div>
          ...
          <div class="stat-card info">
            ...
            <div class="stat-content">
              <div class="stat-value">{{ errorStats.avgRecoveryTime }}ms</div>
              <div class="stat-label">平均恢复时间</div>
            </div>
          </div>
        </div>
          ...
          <div class="custom-error">
            ...
            <div class="custom-form">
              ...
              <select v-model="customError.category" class="form-select">
                <option value="runtime">运行时错误</option>
                <option value="network">网络错误</option>
                <option value="validation">验证错误</option>
                <option value="permission">权限错误</option>
              </select>
              <button @click="simulateCustomError" class="btn btn-primary">
                <Play class="btn-icon" />
                触发错误
              </button>
            </div>
        ...
        <div class="handler-controls">
          <div class="add-handler">
            ...
            <select v-model="newHandler.type" class="form-select">
              <option value="logger">日志记录器</option>
              <option value="reporter">错误报告器</option>
              <option value="recovery">恢复处理器</option>
              <option value="notification">通知处理器</option>
            </select>
            ...
            <button @click="addHandler" class="btn btn-primary" :disabled="!newHandler.name">
              <Plus class="btn-icon" />
              添加处理器
            </button>
          </div>
          ...
          <div class="handler-actions">
            <button @click="testAllHandlers" class="btn btn-secondary">
              <TestTube class="btn-icon" />
              测试所有处理器
            </button>
            <button @click="clearHandlers" class="btn btn-danger">
          ...
          >
            <div class="handler-info">
              <div class="handler-header">
                ...
                <div class="handler-type">{{ getHandlerTypeText(handler.type) }}</div>
            ...
            <div class="handler-controls">
              ...
              <button @click="testHandler(handler)" class="btn btn-sm btn-secondary">
                <TestTube class="btn-icon" />
                测试
              </button>
              <button @click="configureHandler(handler)" class="btn btn-sm btn-primary">
                <Settings class="btn-icon" />
                配置
              </button>
              <button @click="removeHandler(handler)" class="btn btn-sm btn-danger">
                <Trash2 class="btn-icon" />
                删除
              </button>
            </div>
      ...
      <div class="error-logs">
        ...
        <div class="log-list" ref="logContainer">
          ...
          >
            ...
            <div class="log-actions">
              <button @click="retryOperation(log)" class="btn btn-sm btn-secondary">
                <RefreshCw class="btn-icon" />
                重试
              </button>
              <button @click="markAsResolved(log)" class="btn btn-sm btn-success">
                <CheckCircle class="btn-icon" />
                标记已解决
              </button>
              <button @click="reportError(log)" class="btn btn-sm btn-primary">
                <Send class="btn-icon" />
                报告
              </button>
            </div>
        ...
        <div class="strategy-list">
          ...
          >
            ...
            <div class="strategy-controls">
              ...
              <button @click="testStrategy(strategy)" class="btn btn-sm btn-secondary">
                <TestTube class="btn-icon" />
                测试
              </button>
              <button @click="resetStrategy(strategy)" class="btn btn-sm btn-warning">
        ...
        <div class="recovery-monitor">
          ...
          <div class="monitor-stats">
            <div class="monitor-item">
              <div class="monitor-label">总恢复次数</div>
              <div class="monitor-value">{{ recoveryStats.totalRecoveries }}</div>
            </div>
            <div class="monitor-item">
              <div class="monitor-label">平均恢复时间</div>
              <div class="monitor-value">{{ recoveryStats.avgRecoveryTime }}ms</div>
            </div>
            <div class="monitor-item">
              <div class="monitor-label">恢复成功率</div>
              <div class="monitor-value">{{ recoveryStats.successRate }}%</div>
            </div>
          </div>
          ...
          <div class="recovery-timeline">
            ...
            <div class="timeline-list">
              ...
              >
                <div class="timeline-time">{{ formatTime(record.timestamp, true) }}</div>
                <div class="timeline-content">
                  <div class="timeline-strategy">{{ record.strategy }}</div>
                  <div class="timeline-error">{{ record.error }}</div>
...
const simulateCustomError = () => {
  if (!customError.value.message) return
...
const addHandler = () => {
  if (!newHandler.value.name) return
...
const toggleHandler = (handler: any) => {
...
const configureHandler = (handler: any) => {
...
const toggleStack = (log: any) => {
  log.showStack = !log.showStack
}

const toggleContext = (log: any) => {
  log.showContext = !log.showContext
}

const retryOperation = (log: any) => {
  // 模拟重试操作
  const retryLog = {
    id: Date.now(),
    timestamp: Date.now(),
...
const testStrategy = (strategy: any) => {
  // 模拟测试恢复策略
...
const resetStrategy = (strategy: any) => {
...
const getHandlerTypeText = (type: string) => {
  const types = {
    logger: '日志记录器',
    reporter: '错误报告器',
    recovery: '恢复处理器',
