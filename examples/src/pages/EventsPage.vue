...
const filteredEventLog = computed(() => {
  return eventLog.value.filter((event) => {
    if (logFilter.value.type && event.type !== logFilter.value.type)
return false
    if (logFilter.value.level && event.level !== logFilter.value.level)
return false
    if (logFilter.value.search && !event.name.toLowerCase().includes(logFilter.value.search.toLowerCase()))
return false
    return true
  })
})
...
// 方法
function addEventToLog(event: any) {
  const logEntry = {
...
function publishQuickEvent(quickEvent: any) {
  const event = {
...
function toggleListener(listener: any) {
...
function testListener(listener: any) {
  const testEvent = {
      ...
      <div class="event-monitor">
        <div class="monitor-header">
          ...
          <div class="monitor-controls">
            <button class="btn btn-secondary" @click="clearEventLog">
            ...
            <button class="btn btn-secondary" @click="exportEventLog">
      ...
      <div class="event-publisher">
        <div class="publisher-form">
          ...
          <div class="form-actions">
            <button class="btn btn-primary" :disabled="!newEvent.name" @click="publishEvent">
              <Send class="btn-icon" />
              发布事件
            </button>
            <button class="btn btn-secondary" @click="publishBatchEvents">
              <Layers class="btn-icon" />
              批量发布
            </button>
            <button class="btn btn-secondary" @click="scheduleEvent">
    ...
    <div class="section">
      ...
      <div class="listener-manager">
        <div class="manager-header">
          <button class="btn btn-primary" @click="showAddListener = true">
            <Plus class="btn-icon" />
            添加监听器
          </button>
              ...
              <tr v-for="listener in listeners" :key="listener.id">
                ...
                <td>
                  <div class="action-buttons">
                    ...
                    <button class="btn btn-sm btn-primary" @click="testListener(listener)">
                      <Play class="btn-icon" />
                      测试
                    </button>
                    <button class="btn btn-sm btn-danger" @click="removeListener(listener)">
                      <Trash2 class="btn-icon" />
                      删除
                    </button>
                  </div>
    ...
    <div class="section">
      ...
      <div class="event-flow">
        <div class="flow-controls">
          <button class="btn btn-primary" :disabled="isFlowRunning" @click="startEventFlow">
            <Play class="btn-icon" />
            {{ isFlowRunning ? '运行中...' : '开始演示' }}
          </button>
          <button class="btn btn-secondary" :disabled="!isFlowRunning" @click="stopEventFlow">
            <Square class="btn-icon" />
            停止
          </button>
          <button class="btn btn-secondary" @click="resetEventFlow">
        ...
        <div class="flow-legend">
          <div class="legend-item">
            <div class="legend-color publisher" />
            <span>事件发布者</span>
          </div>
          <div class="legend-item">
            <div class="legend-color bus" />
            <span>事件总线</span>
          </div>
          <div class="legend-item">
            <div class="legend-color listener" />
            <span>事件监听器</span>
          </div>
          <div class="legend-item">
            <div class="legend-color triggered" />
            <span>已触发</span>
          </div>
        </div>
      </div>
    ...
    <div v-if="showAddListener" class="modal-overlay" @click="closeAddListener">
      <div class="modal-content" @click.stop>
        <div class="modal-header">
          <h3>添加事件监听器</h3>
          <button class="modal-close" @click="closeAddListener">
            <X class="close-icon" />
          </button>
        </div>
        <div class="modal-body">
          <form @submit.prevent="addListener">
            ...
            <div class="form-actions">
              <button type="button" class="btn btn-secondary" @click="closeAddListener">
                取消
              </button>
