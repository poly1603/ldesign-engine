...
var directiveCompatibility = require('../directives/utils/directive-compatibility.cjs');
var environmentManager = require('../environment/environment-manager.cjs');
var errorManager = require('../errors/error-manager.cjs');
var eventManager = require('../events/event-manager.cjs');
var lifecycleManager = require('../lifecycle/lifecycle-manager.cjs');
var logger = require('../logger/logger.cjs');
var middlewareManager = require('../middleware/middleware-manager.cjs');
var notificationSystem = require('../notifications/notification-system.cjs');
...
var managerRegistry = require('./manager-registry.cjs');
...
class EngineImpl {
  ...
  get events() {
    if (!this._events) {
      ...
      this._events = eventManager.createEventManager(this.logger);
  ...
  get state() {
    if (!this._state) {
      const startTime = Date.now();
      this._state = stateManager.createStateManager(this.logger);
      const initTime = Date.now() - startTime;
  ...
  get errors() {
    if (!this._errors) {
      const startTime = Date.now();
      this._errors = errorManager.createErrorManager();
      const initTime = Date.now() - startTime;
  ...
  get directives() {
    if (!this._directives) {
      const startTime = Date.now();
      this._directives = directiveManager.createDirectiveManager();
      const initTime = Date.now() - startTime;
  ...
  get notifications() {
    if (!this._notifications) {
      const startTime = Date.now();
      this._notifications = notificationSystem.createNotificationSystem(this);
      const initTime = Date.now() - startTime;
  ...
  get middleware() {
    if (!this._middleware) {
      const startTime = Date.now();
      this._middleware = middlewareManager.createMiddlewareManager(this.logger);
      const initTime = Date.now() - startTime;
  ...
  get plugins() {
    if (!this._plugins) {
      const startTime = Date.now();
      this._plugins = pluginManager.createPluginManager(this);
      const initTime = Date.now() - startTime;
  ...
  get cache() {
    if (!this._cache) {
      const startTime = Date.now();
      this._cache = cacheManager.createCacheManager(this.config?.get("cache", {}));
      const initTime = Date.now() - startTime;
  ...
  get performance() {
    if (!this._performance) {
      const startTime = Date.now();
      this._performance = performanceManager.createPerformanceManager(void 0, this);
      const initTime = Date.now() - startTime;
  ...
  get security() {
    if (!this._security) {
      const startTime = Date.now();
      this._security = securityManager.createSecurityManager(void 0, this);
      const initTime = Date.now() - startTime;
  ...
  constructor(config = {}) {
    ...
    this._isReady = false;
    try {
      this.config = configManager.createConfigManager({
        debug: false,
        ...config
      });
      this.config?.setSchema(configManager.defaultConfigSchema);
      const logLevel = this.config?.get("debug", false) ? "debug" : "info";
      this.logger = logger.createLogger(logLevel);
      this.managerRegistry = new managerRegistry.ManagerRegistry(this.logger);
      this.registerManagers();
      this.environment = environmentManager.createEnvironmentManager(this.logger);
      this.lifecycle = lifecycleManager.createLifecycleManager(this.logger);
  ...
  install(app) {
    ...
    if (this._directives) {
      ...
      directiveNames.forEach((name) => {
        ...
        if (eng) {
          const vueDir = directiveCompatibility.convertEngineToVueDirective(eng);
