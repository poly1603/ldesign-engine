...
class DistributedSync {
  constructor(config = {}, logger) {
    ...
    this.localState = vue.reactive({});
    this.remoteState = vue.reactive({});
    this.syncQueue = [];
    this.isSyncing = vue.ref(false);
    this.version = 0;
    this.listeners = /* @__PURE__ */ new Map();
    this.peerId = config.id || this.generatePeerId();
    this.syncStatus = vue.ref({
      connected: false,
      syncing: false,
      lastSync: 0,
      peers: 0,
...
function useDistributedSync(config) {
  ...
  const state = vue.computed(() => sync.getState());
  const status = vue.ref(sync.getStatus());
...
exports.createDistributedSync = createDistributedSync;
exports.useDistributedSync = useDistributedSync;
//# sourceMappingURL=distributed-sync.cjs.map
