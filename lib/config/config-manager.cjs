...
class ConfigManagerImpl {
  ...
  get(path, defaultValue) {
    const value = index.getNestedValue(this.config, path);
  ...
  triggerMergeWatchers(oldConfig, newConfig) {
    ...
    for (const key of allKeys) {
      const oldValue = index.getNestedValue(oldConfig, key);
      const newValue = index.getNestedValue(newConfig, key);
  ...
  deleteNestedValue(obj, path) {
    ...
    for (let i = 0; i < keys.length - 1; i++) {
      const key = keys[i];
      if (!(key in current) || !index.isObject(current[key])) {
        return;
      }
      current = current[key];
    }
  ...
  deepMerge(target, source) {
    for (const key in source) {
      if (source[key] && index.isObject(source[key])) {
        if (!target[key] || !index.isObject(target[key])) {
          target[key] = {};
        }
  ...
  validateConfig(config, schema, basePath = "") {
    ...
    for (const [key, schemaItem] of Object.entries(schema)) {
      ...
      const value = index.getNestedValue(config, fullPath);
      ...
      if (schemaItem.type === "object" && schemaItem.children && index.isObject(value)) {
  ...
  buildDefaultConfig(config, schema, basePath = "") {
    for (const [key, schemaItem] of Object.entries(schema)) {
      ...
      if (schemaItem.default !== void 0) {
        index.setNestedValue(config, fullPath, schemaItem.default);
      }
      if (schemaItem.children) {
...
exports.NamespacedConfigManager = NamespacedConfigManager;
exports.createConfigManager = createConfigManager;
exports.defaultConfigSchema = defaultConfigSchema;
//# sourceMappingURL=config-manager.cjs.map
