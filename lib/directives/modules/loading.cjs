...
class LoadingDirective extends directiveBase.DirectiveBase {
  constructor() {
    super({
      name: "loading",
      description: "\u663E\u793A\u52A0\u8F7D\u72B6\u6001",
  ...
  addLoading(el, config) {
    ...
    vueDirectiveAdapter.directiveUtils.storeData(el, "loading-overlay", overlay);
  }
  removeLoading(el) {
    const overlay = vueDirectiveAdapter.directiveUtils.getData(el, "loading-overlay");
    if (overlay && overlay.parentNode) {
      overlay.parentNode.removeChild(overlay);
    }
    document.body.style.overflow = "";
    vueDirectiveAdapter.directiveUtils.removeData(el, "loading-overlay");
  }
...
const vLoading = vueDirectiveAdapter.defineDirective(new LoadingDirective());
