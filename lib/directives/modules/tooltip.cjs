...
class TooltipDirective extends directiveBase.DirectiveBase {
  constructor() {
    super({
      name: "tooltip",
      description: "\u663E\u793A\u5DE5\u5177\u63D0\u793A",
  ...
  setupTooltip(el, config) {
    ...
    const showTooltip = () => {
      ...
      showTimer = window.setTimeout(() => {
        ...
        vueDirectiveAdapter.directiveUtils.storeData(el, "tooltip-element", tooltip);
        requestAnimationFrame(() => {
          tooltip.style.opacity = "1";
          tooltip.style.transform = "scale(1)";
        });
      }, delay);
      vueDirectiveAdapter.directiveUtils.storeData(el, "tooltip-show-timer", showTimer);
    };
    const hideTooltip = () => {
      if (showTimer) {
        clearTimeout(showTimer);
        showTimer = null;
        vueDirectiveAdapter.directiveUtils.removeData(el, "tooltip-show-timer");
      }
      const tooltip = vueDirectiveAdapter.directiveUtils.getData(el, "tooltip-element");
      if (!tooltip)
        return;
      hideTimer = window.setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          vueDirectiveAdapter.directiveUtils.removeData(el, "tooltip-element");
        }, 200);
      }, 100);
      vueDirectiveAdapter.directiveUtils.storeData(el, "tooltip-hide-timer", hideTimer);
    };
    const handlers = {
      show: showTooltip,
      hide: hideTooltip
    ...
    vueDirectiveAdapter.directiveUtils.storeData(el, "tooltip-handlers", handlers);
    vueDirectiveAdapter.directiveUtils.storeData(el, "tooltip-trigger", trigger);
  }
  cleanupTooltip(el) {
    const handlers = vueDirectiveAdapter.directiveUtils.getData(el, "tooltip-handlers");
    const trigger = vueDirectiveAdapter.directiveUtils.getData(el, "tooltip-trigger");
    const tooltip = vueDirectiveAdapter.directiveUtils.getData(el, "tooltip-element");
    const showTimer = vueDirectiveAdapter.directiveUtils.getData(el, "tooltip-show-timer");
    const hideTimer = vueDirectiveAdapter.directiveUtils.getData(el, "tooltip-hide-timer");
    if (showTimer) {
      clearTimeout(showTimer);
    }
    if (hideTimer) {
    ...
    vueDirectiveAdapter.directiveUtils.removeData(el, "tooltip-element");
    vueDirectiveAdapter.directiveUtils.removeData(el, "tooltip-handlers");
    vueDirectiveAdapter.directiveUtils.removeData(el, "tooltip-trigger");
    vueDirectiveAdapter.directiveUtils.removeData(el, "tooltip-show-timer");
    vueDirectiveAdapter.directiveUtils.removeData(el, "tooltip-hide-timer");
  }
  ...
  positionTooltip(el, tooltip, placement, offset) {
...
const vTooltip = vueDirectiveAdapter.defineDirective(new TooltipDirective());
