...
class CopyDirective extends directiveBase.DirectiveBase {
  constructor() {
    super({
      name: "copy",
  ...
  mounted(el, binding) {
    ...
    vueDirectiveAdapter.directiveUtils.storeData(el, "copy-handler", handleClick);
  ...
  updated(el, binding) {
    const oldHandler = vueDirectiveAdapter.directiveUtils.getData(el, "copy-handler");
    if (oldHandler) {
  ...
  unmounted(el) {
    const handler = vueDirectiveAdapter.directiveUtils.getData(el, "copy-handler");
    if (handler) {
      el.removeEventListener("click", handler);
      vueDirectiveAdapter.directiveUtils.removeData(el, "copy-handler");
    }
    el.style.cursor = "";
...
const vCopy = vueDirectiveAdapter.defineDirective(new CopyDirective());

exports.CopyDirective = CopyDirective;
