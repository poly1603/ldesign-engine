...
class ResizeDirective extends directiveBase.DirectiveBase {
  constructor() {
    super({
      name: "resize",
  ...
  mounted(el, binding) {
    ...
    vueDirectiveAdapter.directiveUtils.storeData(el, "resize-observer", observer);
    vueDirectiveAdapter.directiveUtils.storeData(el, "resize-config", config);
    observer.observe(el);
    this.log("Resize observer attached", el);
  }
  updated(el, binding) {
    const newConfig = this.parseConfig(binding);
    const oldConfig = vueDirectiveAdapter.directiveUtils.getData(el, "resize-config");
  ...
  unmounted(el) {
    const observer = vueDirectiveAdapter.directiveUtils.getData(el, "resize-observer");
    const timer = vueDirectiveAdapter.directiveUtils.getData(el, "resize-timer");
    if (timer) {
      clearTimeout(timer);
      vueDirectiveAdapter.directiveUtils.removeData(el, "resize-timer");
    }
    if (observer) {
      observer.unobserve(el);
      observer.disconnect();
      vueDirectiveAdapter.directiveUtils.removeData(el, "resize-observer");
    }
    vueDirectiveAdapter.directiveUtils.removeData(el, "resize-config");
    ...
    const callback = (entries) => {
      ...
      for (const entry of entries) {
        if (config.debounce && config.debounce > 0) {
          ...
          vueDirectiveAdapter.directiveUtils.storeData(el, "resize-timer", newTimer);
        } else {
          config.callback?.(entry);
          this.emitEvent(el, "resize", {
            width: entry.contentRect.width,
...
const vResize = vueDirectiveAdapter.defineDirective(new ResizeDirective());

exports.ResizeDirective = ResizeDirective;
exports.default = vResize;
exports.vResize = vResize;
//# sourceMappingURL=resize.cjs.map
