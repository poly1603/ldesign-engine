...
var vue = require('vue');
var useEngine = require('./useEngine.cjs');

function useNotification() {
  const engine = useEngine.useEngine();
  const show = (options) => {
...
function useCache(key, defaultValue) {
  ...
  const value = vue.ref(cache.get(key) || defaultValue);
...
function usePerformance() {
  ...
  const isMonitoring = vue.ref(false);
  ...
  return {
    metrics: vue.computed(() => metrics.value),
    isMonitoring: vue.computed(() => isMonitoring.value),
    startMonitoring,
    stopMonitoring,
    measure,
    mark
...
function useConfig(path, defaultValue) {
  ...
  const value = vue.ref(config.get(path, defaultValue));
  const set = (newValue) => {
...
function useErrorHandler() {
  ...
  return {
    errors: vue.computed(() => errorList.value),
    handle,
    capture,
    clearErrors
  };
...
function usePlugins() {
  ...
  const installedPlugins = vue.computed(() => plugins.getInstalledPlugins());
  const pluginCount = vue.computed(() => installedPlugins.value.length);
...
//# sourceMappingURL=useEngineFeatures.cjs.map
